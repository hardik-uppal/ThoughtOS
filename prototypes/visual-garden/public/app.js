...
function aroundCard({ note, linked, tags, semantic }) {
  const label = linked ? describeLink(notes, state.selected, note.id).replace('Unnamed connection', 'Connected') : semantic ? 'Not connected · similar meaning' : tags.length ? `Not connected · #${tags[0]}` : 'Not connected · from elsewhere';
  return `<article class="around-card ${linked ? 'is-linked' : 'nearby-card'} ${color(note)}"><button class="around-open ${linked ? 'branch-card' : ''}" data-hop="${esc(note.id)}"><span class="relation-caption">${esc(label)}</span><strong>${esc(note.title)}</strong><span class="around-excerpt">${esc(note.body.split('\n\n')[0])}</span></button><div class="around-actions"><button data-read-other="${esc(note.id)}" aria-label="Read ${esc(note.title)}">Read</button>${linked ? `<span>↔</span>` : `<button class="suggest-connect" data-suggest-connect="${esc(note.id)}" aria-label="Connect to ${esc(note.title)}">＋ Connect</button>`}</div></article>`;
}
function renderChain() {
  const note = selected(), links = related(notes, note.id);
  const items = surroundingNotes(notes, note.id, { scope: state.neighbors, vectors: prefs.semantic ? vectors : {} });
  return `<div class="chain-board"><article class="chain-focus ${color(note)}"><div class="focus-top"><button id="all-cards">← All notes</button><span>${esc(note.topic)}</span></div><h2 tabindex="-1">${esc(note.title)}</h2><div class="focus-body">${esc(note.body)}</div><div class="focus-tags">${note.tags.map(t => `<button data-tag="${esc(t)}">#${esc(t)}</button>`).join('')}</div><div class="focus-actions"><button data-edit="${esc(note.id)}">Edit</button><button data-read="${esc(note.id)}">Read / connections</button><button data-connect="${esc(note.id)}">＋ Connect</button></div></article><section class="surroundings" aria-label="Notes around this thought"><div class="around-heading"><span>Around this thought</span><div class="neighbor-scopes" role="group" aria-label="Nearby notes"><button data-neighbors="all" aria-pressed="${state.neighbors === 'all'}">All <small>${notes.length - 1}</small></button><button data-neighbors="unlinked" aria-pressed="${state.neighbors === 'unlinked'}">Unconnected <small>${notes.length - 1 - links.length}</small></button><button data-neighbors="linked" aria-pressed="${state.neighbors === 'linked'}">Connected <small>${links.length}</small></button></div></div><div class="around-grid">${items.map(aroundCard).join('')}</div>${!items.length ? '<p class="small-muted">Nothing in this view yet. Choose All to keep wandering.</p>' : ''}</section></div>`;
}
